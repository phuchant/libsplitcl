...
unsigned optMergeDeviceID = 0;
unsigned optNbSkipIter = 0;
int optBuffMgr = BufferManager::OPTIM;
...
static option opts[] = {
  ...
  {"BUFFMGR", "Buffer manager: SIMPLE or OPTIM.", false, buffmgrOption},
  ...
  {"MERGEDEV", "The ID of the device onto the merge occurs.",
   false, mergedevOption},
  {"NBSKIPITER", "Number of iterations to skip before splitting the kernel.",
   false, nbskipiterOption},
  {"SCHED", "Scheduler (AKGR, FIXEDPOINT, BROYDEN, BADBROYDEN, MKGR, ENV",
   false, schedOption},
  {"NOMEMCPY", "(not safe)", false, nomemcpyOption},
  {"LOGMAXITER", "Build a log graph and stop the execution after LOGMAXITER " \
   "iterations.",
   false, logmaxiterOption},
...
static void buffmgrOption(char *env) {
  if (env && !strcmp(env, "SIMPLE"))
    optBuffMgr = BufferManager::SIMPLE;
}
...
static void schedOption(char *env) {
  if (env) {
    if (!strcmp(env, "AKGR")) {
      optScheduler = Scheduler::AKGR;
    } else if (!strcmp(env, "MKGR")) {
      optScheduler = Scheduler::MKGR;
    } else if (!strcmp(env, "FIXPOINT")) {
      optScheduler = Scheduler::FIXPOINT;
    } else if (!strcmp(env, "BROYDEN")) {
      optScheduler = Scheduler::BROYDEN;
    } else if (!strcmp(env, "BADBROYDEN")) {
...
static void logmaxiterOption(char *env) {
  ...
  optLogMaxIter = atoi(env);
  DebugFlag = true;
  CurrentDebugType.push_back(std::string("log"));
}
